...
if spec is None or not spec.origin:
    raise ImportError("Can't find the module '_twofish'")

_twofish = cdll.LoadLibrary(spec.origin)
...
_Twofish_prepare_key.argtypes = [
    c_char_p,  # uint8_t key[]
    c_int,  # int key_len
...
def _ensure_bytes(data):
    if (IS_PY2 and not isinstance(data, str)) or (
        not IS_PY2 and not isinstance(data, bytes)
    ):
